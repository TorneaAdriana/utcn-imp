...
{
  std::shared_ptr<Expr> term = ParseMulSubExpr();
  while (Current().Is(Token::Kind::PLUS)) {
    lexer_.Next();
    auto rhs = ParseMulSubExpr();
...
//equal
std::shared_ptr<Expr> Parser::ParseEqualSubExpr()
{
...
//multiply
std::shared_ptr<Expr> Parser::ParseMulSubExpr()
{
